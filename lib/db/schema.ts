...
// User aggregates table for fast usage lookups
export const userAggregates = pgTable("UserAggregates", {
  userId: uuid("userId")
    .primaryKey()
    .notNull()
    .references(() => user.id),
  totalTokens: integer("totalTokens").notNull().default(0),
  totalCostCents: integer("totalCostCents").notNull().default(0),
  monthlyTokens: integer("monthlyTokens").notNull().default(0),
  monthlyMessages: integer("monthlyMessages").notNull().default(0),
  dailyTokens: integer("dailyTokens").notNull().default(0),
  dailyMessages: integer("dailyMessages").notNull().default(0),
...
export type UserAggregates = InferSelectModel<typeof userAggregates>;
