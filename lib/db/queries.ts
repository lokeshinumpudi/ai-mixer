...
export async function listCompareRunsByChat({
  ...
  try {
    ...
    const resultsByRunId = allResults.reduce((acc, result) => {
...
export async function batchInsertChatUsage(records: UsageRecord[]) {
  try {
    ...
    // 1. Insert detailed usage records (existing)
    await db.insert(chatUsage).values(records);

    // 2. Update aggregates by user (NEW)
    ...
    for (const record of records) {
      ...
      const existing = userUpdates.get(key) || {
        tokens: 0,
        cost: 0,
        messages: 0,
      };
      ...
      userUpdates.set(key, {
        tokens: existing.tokens + record.tokensIn + record.tokensOut,
        cost: existing.cost + record.cost,
        messages: existing.messages + 1,
      });
    }

    // Update aggregates for each user
    for (const [userId, deltas] of userUpdates) {
      await updateUserAggregate(userId, {
        tokens: deltas.tokens,
        costCents: deltas.cost,
...
// Update user aggregates when usage is recorded
export async function updateUserAggregate(
  userId: string,
  deltas: {
    tokens: number;
    costCents: number;
  ...
  try {
    await db
      .insert(userAggregates)
      .values({
        userId,
        totalTokens: deltas.tokens,
        totalCostCents: deltas.costCents,
        monthlyTokens: deltas.tokens,
        monthlyMessages: deltas.messages,
        dailyTokens: deltas.tokens,
        dailyMessages: deltas.messages,
        monthStart: monthStart.toISOString().split("T")[0],
        dayStart: dayStart.toISOString().split("T")[0],
      })
      .onConflictDoUpdate({
        target: userAggregates.userId,
        set: {
          totalTokens: sql`${userAggregates.totalTokens} + ${deltas.tokens}`,
          totalCostCents: sql`${userAggregates.totalCostCents} + ${deltas.costCents}`,
          monthlyTokens: sql`${userAggregates.monthlyTokens} + ${deltas.tokens}`,
          monthlyMessages: sql`${userAggregates.monthlyMessages} + ${deltas.messages}`,
          dailyTokens: sql`${userAggregates.dailyTokens} + ${deltas.tokens}`,
          dailyMessages: sql`${userAggregates.dailyMessages} + ${deltas.messages}`,
          updatedAt: now,
        },
      });
  } catch (error) {
...
// Helper functions for time-based resets
async function resetMonthlyAggregate(userId: string) {
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
...
async function resetDailyAggregate(userId: string) {
  const now = new Date();
  const dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
...
// Fast O(1) lookup for usage limits using aggregates
export async function getUserUsageAndLimitsOptimized(
  userId: string,
  userType: UserType
) {
  try {
    // Fast O(1) lookup instead of expensive aggregation
    ...
    if (!aggregate) {
      // User has no usage yet, return zeros
      const entitlements = entitlementsByUserType[userType];
      return {
        used: 0,
        quota: entitlements.maxMessagesPerDay || 0,
        remaining: entitlements.maxMessagesPerDay || 0,
        isOverLimit: false,
        type: "daily" as const,
        resetInfo: "tomorrow at 5:29 AM",
      };
    ...
    if (aggregate.monthStart !== currentMonth) {
      await resetMonthlyAggregate(userId);
      aggregate.monthlyTokens = 0;
      aggregate.monthlyMessages = 0;
    }
    if (aggregate.dayStart !== currentDay) {
      await resetDailyAggregate(userId);
      aggregate.dailyTokens = 0;
      aggregate.dailyMessages = 0;
    }

    const entitlements = entitlementsByUserType[userType];

    // Use appropriate limits based on user type
    let used: number;
    let quota: number;
    let type: "daily" | "monthly";
    let resetInfo: string;

    if (userType === "anonymous" || userType === "free") {
      // Daily limits for anonymous and free users
      used = aggregate.dailyMessages;
      quota = entitlements.maxMessagesPerDay ?? 0;
      type = "daily";
      resetInfo = "tomorrow at 5:29 AM";
    } else {
      // Monthly limits for pro users
      used = aggregate.monthlyMessages;
      quota = entitlements.maxMessagesPerMonth ?? 0;
      type = "monthly";
      resetInfo = "monthly at 5:29 AM";
    }
...
export async function getUserUsageWithValidation(
  ...
  try {
    // Get raw data for client computation (still needed for detailed history)
    const usageData = await getUserUsageData(userId, limit, page);

    // Get server validation data using optimized aggregates
    const usageInfo = await getUserUsageAndLimitsOptimized(userId, userType);

    // Get current usage from aggregates (FAST O(1) lookup)
    const currentUsage = await getCurrentUserUsageOptimized(userId);
...
// NEW: Optimized current usage lookup using aggregates
export async function getCurrentUserUsageOptimized(userId: string) {
  try {
    // Fast O(1) lookup from aggregates table
    const [aggregate] = await db
      .select()
      .from(userAggregates)
      .where(eq(userAggregates.userId, userId))
    ...
    // Get chat counts (still need to query for this)
    ...
    return {
      ...
      totalChats: Number(chatCounts.totalChats) || 0,
      activeChats: Number(chatCounts.activeChats) || 0,
    };
  } catch (error) {
    throw new ChatSDKError(
      "bad_request:database",
